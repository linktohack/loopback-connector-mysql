...
MySQL.prototype.buildExpression = function(columnName, operator, operatorValue,
  ...
  if (operator === 'near') {
    ...
    var radius = EARTH_RADIUS[operatorValue.unit || 'miles'];
    var clause1 = `(
                     ${radius} * ACOS(
                       COS(RADIANS(?))
                       * COS(RADIANS(ST_X(${columnName})))
                       * COS(RADIANS(ST_Y(${columnName})) - RADIANS(?))
                       + SIN(RADIANS(?))
                       * SIN(RADIANS(ST_X(${columnName})))
                     )
                   ) < ?`.split('\n').map(it => it.trim()).join(' ');
    return new ParameterizedSQL(clause1, [operatorValue.near.lat, operatorValue.near.lng, operatorValue.near.lat, operatorValue.maxDistance]);
  }
...
MySQL.prototype._buildWhere = function(model, where, tableAlias, aliases) {
  ...
  for (var key in where) {
    ...
    if (p == null) {
      ...
      if (relations && relations[key]) {
        ...
        let childWhere = self._buildWhere(relation.model, where[key],
          aliases[relation.model + '|' + relation.foreignKey], aliases);
    ...
    if (expression === null || expression === undefined) {
      ...
      } else if (operator === 'regexp' && typeof expression === 'string') {
        // do not coerce RegExp based on property definitions
        var val = toRegExp(expression);
  ...
  for (var key in where) {
    ...
    if (p == null) {
      ...
      if (relations && relations[key]) {
        ...
        relation.foreignKey = relation.foreignKey !== '' ? relation.foreignKey :
          (type === 'hasOne' || type === 'hasMany') ? lowerCaseFirstLetter(model) + 'Id' :
            lowerCaseFirstLetter(relation.model) + 'Id';
        aliases[relation.model + '|' + relation.foreignKey] = childTableAlias;
        if (type === 'belongsTo') {
          joinOn = tableAlias + '.' + relation.foreignKey + ' = ' + childTableAlias + '.id';
          ...
          aliases[relation.through + '|' + relation.foreignKey] = throughModelAlias;
          ...
          joinOn = childTableAlias + '.' + relation.foreignKey + ' = ' + tableAlias + '.id';
        }
    ...
    if (t.length === 1) {
      ...
      if (key.indexOf('.') > -1) {
        ...
        if (relations && relations[modelAndProperty[0]]) {
          ...
          const alias = aliases[relation.model + '|' + relation.foreignKey];
          if (alias) {
